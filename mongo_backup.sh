...
S3_BUCKET_NAME="submissionstorage" #replace with your bucket name on Amazon S3
S3_BUCKET_PATH="mongodb-backups"

# Create backup, with locks to force file syncronization and lock writes
mongo $MONGO_DATABASE -u $ADMIN_USER -p $ADMIN_PASSWORD --eval "db.fsyncLock()"
$MONGODUMP_PATH --host $MONGO_HOST:$MONGO_PORT --username $ADMIN_USER --password $ADMIN_PASSWORD --db $MONGO_DATABASE
mongo $MONGO_DATABASE -u $ADMIN_USER -p $ADMIN_PASSWORD --eval "db.fsyncUnlock()"
...
# Upload to S3
s3cmd put $BACKUPS_DIR/$BACKUP_NAME.tgz s3://$S3_BUCKET_NAME/$S3_BUCKET_PATH/$BACKUP_NAME.tgz
